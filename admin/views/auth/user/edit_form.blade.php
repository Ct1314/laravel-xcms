...
<div class="form-group">
    ...
    <div class="col-sm-8">
        <div class="input-group">
            ...
            <input type="text" id="username" name="username"  value="{{ $user->username }}" class="form-control" placeholder="请输入登陆用户名">
        </div>
    </div>
</div>
<div class="form-group">
    ...
    <div class="col-sm-8">
        <div class="input-group">
            <span class="input-group-addon"><i class="fa fa-pencil"></i></span>
            <input type="text" id="email" name="email" value="{{ $user->email }}" class="form-control" placeholder="请输入邮箱">
        </div>
    </div>
</div>
<div class="form-group">
    ...
    <div class="col-sm-8">
        <div class="input-group">
            <span class="input-group-addon"><i class="fa fa-pencil"></i></span>
            <input type="text" id="name" name="name"  value="{{ $user->name }}" class="form-control required" placeholder="请输入昵称">
        </div>
    </div>
</div>
<div class="form-group">
                ...
                @if($top['child'])
                    <optgroup label="{{ $top['name'] }}">
                        @foreach($top['child'] as $sub)
                            <option value="{{ $sub['id'] }}"
                            {{ in_array($sub['id'],$user_permissions)?"selected":"" }}
                            >{{ $sub['name'] }}</option>
                        @endforeach
                    </optgroup>
                @else
                    <option value="{{ $top['id'] }}"
                        {{ in_array($top['id'],$user_permissions)?"selected":"" }}
                    >{{ $top['name'] }}</option>
                @endif
            @empty
            @endforelse
